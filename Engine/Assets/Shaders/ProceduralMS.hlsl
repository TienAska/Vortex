...
void main(in payload Payload payload, out indices uint3 indices[1], out vertices MeshToPixel vertices[3])
{
    ...
    vertices[0].PositionHS = mul(float4(0, 0.25, 0, 1), global.model);
    vertices[0].Color = float4(1, 0, 0, 1);

    vertices[1].PositionHS = mul(float4(0.25, -0.25, 0, 1), global.model);
    vertices[1].Color = float4(0, 1, 0, 1);

    vertices[2].PositionHS = mul(float4(-0.25, -0.25, 0, 1), global.model);
